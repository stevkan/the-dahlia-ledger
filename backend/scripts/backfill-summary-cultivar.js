...
const recordsByCultivar = new Map(
  recordsSnap.docs.map((doc) => [doc.id, doc.data().core?.cultivar]),
)

const toUpdate = summariesSnap.docs.filter((doc) => {
  const storedCultivar = doc.data().core?.cultivar
  const sourceCultivar = recordsByCultivar.get(doc.id)
  return sourceCultivar !== undefined && storedCultivar !== sourceCultivar
})
...
for (const doc of toUpdate) {
  const cultivar = recordsByCultivar.get(doc.id)
  console.log(`${DRY_RUN ? '[dry-run] ' : ''}${doc.id} -> core.cultivar: ${cultivar ?? '(none)'}`)
  if (!DRY_RUN) {
    batch.update(doc.ref, { 'core.cultivar': cultivar ?? null })
    pendingWrites += 1
