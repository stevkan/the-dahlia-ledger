...
- Added a break-glass App Check recovery path: `window.getAppCheckDebugToken()` and `window.generateAppCheckDebugToken()` (`frontend/src/firebase.ts`), callable from the browser devtools console while signed in. If App Check itself is broken (e.g. the registered debug token was revoked/expired), `authHeaders()` fails trying to attach an App Check token to *every* request — including calls to the two bootstrap endpoints (`GET`/`POST /api/app-check/debug-token[...]`) that exist specifically to recover from this, since they're exempted from App Check on the backend (`APP_CHECK_BOOTSTRAP_PATHS` in `backend/src/server.js`) but the frontend didn't know that. That made the normal Settings → Firebase Token → Generate new token UI unreachable in exactly the situation it's meant to fix. The new console functions call `fetch()` directly with only the Firebase ID token, bypassing `getAppCheck()`/`getToken()` entirely, so they work regardless of App Check's state. They force a fresh ID token (`getIdToken(true)`) rather than trusting a cached one, since a session that's been failing for a while may not have had anything else prompt a refresh. Also added `window.debugAppCheckConfig()` to report what the loaded bundle actually resolved the App Check site key/token state to, for telling a stale/cached bundle apart from a genuine config problem. Documented all three in `.claude/skills/run-the-dahlia-ledger/SKILL.md` and the Settings → Firebase Token hint text.
- The backend's `/api` auth middleware (`backend/src/server.js`) previously collapsed any failure — an invalid ID token, a failed Firestore `upsertKnownUser` write, or a bad App Check token — into the same generic message with no server-side log line, making this kind of failure unnecessarily hard to diagnose. It now logs the real error (`console.error` plus `trackException`) and returns a distinct `500 internal_error` for an `upsertKnownUser` failure instead of misreporting it as an invalid token.

## 0.34.0 - 2026-07-22

- Non-admin garden members can now remove themselves from a garden: `removeGardenMember` (`backend/src/gardens.js`) previously required `owner`/`admin` role for any removal, blocking a viewer/editor from leaving a garden they no longer wanted access to. Self-removal (matched by `userId`) is now allowed regardless of role, while removing another member still requires `owner`/`admin` and a sole owner still cannot remove themselves. `frontend/src/ui/GardenManagementModal.tsx`'s existing "Remove Selected" flow now detects a self-removal and switches the modal (and, via a `gardens` query invalidation added to `deleteGardenMember` in `frontend/src/hooks/useGardens.ts`, the whole app's active garden) to another accessible garden instead of trying to refresh a garden the user just lost access to.
- Added self-service account deletion: a new "Delete Account" control in the Settings modal's Account blade (`frontend/src/ui/SettingsModal.tsx`) calls a new `DELETE /api/users/me` endpoint (`backend/src/routes/users.js`, orchestrated by new `backend/src/accountDeletion.js`) that removes the caller from every garden (`removeUserFromAllGardens` in `backend/src/gardens.js`, unlike the admin-only known-user delete this bypasses the "owns a garden" / "added by another user" protections since it's the user's own account) and deletes their Known Users record, then signs them out client-side. This is permanent for anything the caller solely owned: `removeUserFromAllGardens` now detects gardens left with zero remaining `owner`-role members and deletes them entirely — the garden itself, its dahlia records (Postgres, via the same per-record `deleteRecord` path used elsewhere so photo embeddings are pruned too), maintenance reminders, and order items (any parent order left with no items anywhere, via new `deleteOrderItemsForGarden` in `backend/src/orders.js`, is deleted too) — including the caller's own default/personal garden, so re-signing in with the same login starts fresh rather than silently reclaiming the old one. The Settings copy now says so explicitly.
- Fixed `getGardenUsageCounts` (`backend/src/gardens.js`), which backs the manual "Delete Garden" in-use safeguard, still counting records via a stale Firestore `dahliaRecords` query left over from before the 0.33.0 Postgres migration — it now counts `dahlia_records` rows directly, so the safeguard actually reflects live data again.

## 0.33.0 - 2026-07-21

- Phase 1 of the Firestore/Firebase Storage migration: `dahliaRecords` and `dahliaRecordSummaries` now live in Postgres (`dahlia_records`, `backend/migrations/`), and record/cultivar photo uploads go to Azure Blob Storage (`backend/src/blobStorage.js`) instead of Firebase Storage. Firebase Auth and App Check are unchanged. This replaces the non-atomic dual Firestore write in `backend/src/records.js` (`writeRecordSummary` alongside every record write, the root cause of a real data-consistency bug where the Records list view could silently show stale data after a maintenance script touched `dahliaRecords` without also updating `dahliaRecordSummaries`) with a single write to `dahlia_records`; the "summary" list view is now a live JS projection (`toRecordSummary()`, unchanged) over a column-projected `SELECT`, so there is no second table left to fall out of sync. `updateCultivarPhoto`/`updateCultivarPhotoDefault`/`deleteCultivarPhoto`'s per-record fan-out writes are now wrapped in a real Postgres transaction instead of an unguarded `Promise.all`, and `createRecord`'s record-number assignment now uses `pg_advisory_xact_lock` to close a latent race that existed even under Firestore.
- One-time migration tooling (dry-run by default, `--apply` required to write, refuses to run twice without `--force`): `backend/scripts/migrate-records-to-postgres.js` copies `dahliaRecords`/`dahliaRecordSummaries` into Postgres verbatim, with no reconciliation — the existing drift between the two is preserved on purpose, in a new frozen `dahlia_record_summaries_snapshot` table, so it can be manually audited against the physical garden instead of being silently resolved by the migration. `backend/scripts/migrate-photos-to-azure-blob.js` copies existing photo blobs from Firebase Storage to Azure Blob Storage and records each photo's new `imageBlobPath`/`thumbnailBlobPath`/`listThumbnailBlobPath` (`DahliaPhotoSchema` in `backend/src/schema.js`), retiring the URL-reverse-parsing `publicStorageObjectName()` hack that `backend/src/photos.js` previously needed to re-derive a Firebase object path from a stored public URL.
- Added an admin-only "Data Audit" blade to Settings (`frontend/src/ui/SettingsModal.tsx`, gated on the same `isGlobalAdmin` prop as the Firebase Token blade), backed by `GET /api/records/audit/drift` and `POST /api/records/audit/drift/:id/reviewed` (`backend/src/routes/records.js`, `listRecordDrift`/`markRecordDriftReviewed` in `backend/src/records.js`). It lists every field where the frozen snapshot disagrees with the live record, lets an admin open the record (via a new `openRecordById` in `frontend/src/hooks/useRecords.ts`) to check it against the garden and fix it through the normal Records modal, then mark it reviewed so it drops off the list — without an explicit review flag, a corrected record wouldn't necessarily match the frozen snapshot either and would stay listed forever.
- New required env vars documented in `backend/.env.example`: `DATABASE_URL` (Postgres) and `AZURE_STORAGE_CONNECTION_STRING`/`AZURE_STORAGE_PHOTOS_CONTAINER` (Blob Storage). Added a root `docker-compose.yml` for a local Postgres instance matching `DATABASE_URL`'s default, and `node-pg-migrate` for schema migrations (`npm run migrate:up` in `backend/`). The other ~15 Firestore collections (gardens, orders, companies, assets, reminders, embeddings, etc.) and non-photo Storage usage (asset/order invoice PDFs) are unchanged in this phase.

## 0.32.2 - 2026-07-21

- Fixed photo identification becoming unreliable (intermittent 400/503/504 responses, and unrelated requests like maintenance reminders timing out too) right after enabling `PHOTO_MATCH_PIPELINE=dino` in production. Only the legacy CLIP embedding model was warmed at server startup (`backend/src/server.js`); SlimSAM segmentation and DINOv2 were left to lazy-load inside the first live `identify-photo` request, blocking Node's single event loop for the whole process while they downloaded and initialized, and retrying that same expensive cold start from scratch on every subsequent request if it ever failed (`backend/src/modelMemo.js` intentionally doesn't cache load failures). The server now warms SlimSAM and DINOv2 at startup instead of CLIP.
- Removed the CLIP-based zero-shot form classification step from the segmented-DINOv2 matching path (`identifyPhotoDino` in `backend/src/agent.js`) so a live identify-photo call under `PHOTO_MATCH_PIPELINE=dino` never loads the CLIP model into memory — only SAM and DINOv2 are needed. This drops a marginal (0.004) form-match scoring boost; the legacy CLIP pipeline (used only as a fallback for gardens not yet backfilled onto the new embeddings) is unchanged.
- `identifyPhoto()` (`frontend/src/api/client.ts`) now aborts after 45s like the app's other network calls, instead of hanging indefinitely with no feedback if the request stalls.
