...
- **App Check 403s can lock out the normal recovery UI too.** If the *registered* debug token itself is stale/revoked, `authHeaders()` (`frontend/src/firebase.ts`) fails trying to attach an App Check token to every request — including calls the Settings > Firebase Token blade makes to fetch/regenerate the token, so that UI becomes unreachable in exactly the situation it's meant to fix. From the browser devtools console while signed in, run `await getAppCheckDebugToken()` to see the current stored token, or `await generateAppCheckDebugToken()` to mint a new one (global admin only) — both bypass App Check entirely by hitting the backend's exempted bootstrap endpoints directly. Register whichever token you use in Firebase Console > App Check > Manage debug tokens, then reload.
