...
function isCapabilitiesQuestion(value: string) {
  ...
  const namesAgent = /\b(agent helper|agent|you)\b/.test(normalized)
...
export function AgentPanel({
  ...
  const [result, setResult] = useState<string | null>(() => localCapabilitiesMessage())
  ...
  async function submit() {
    ...
    if (isCapabilitiesQuestion(prompt)) {
      setResult(localCapabilitiesMessage())
      setText('')
      onReview(null)
      return
    }
  ...
  return (
    <div className="agent">
      <div className="muted">
        Agent Helper can answer dahlia questions and use saved records for reviews, coaching, maintenance prompts, and summaries.
      </div>
...
export function AnalyticsPanel({
  ...
  function defaultChartTypeForMetric(nextMetric: AnalyticsMetric): AnalyticsChartType {
    ...
    if (nextMetric === 'invoice_total_by_season') return 'line'
    return 'bar'
  }
